...
const Login = () => {
  ...
  const handleLogIn = async (e) => {
    ...
    try {
      ...
      // Log the result to check the data
      console.log("Sign-in result:", result);
  
      // Check if user data exists in the result
      if (!result?.user) {
        throw new Error('Login failed: No user data returned');
      }
  
      // Save user data in database
      await saveUser(result.user);  // Assuming saveUser function handles the user data properly
    ...
    <div className="flex justify-center items-center min-h-screen">
      <div className="flex flex-col max-w-md p-6 rounded-md sm:p-10 bg-gray-100 text-gray-900">
        ...
        <form onSubmit={handleLogIn} className="space-y-6">
          ...
          <div>
            <button type="submit" className="bg-secondary w-full rounded-md py-3 text-white">
        ...
        <p className="px-6 text-sm text-center text-gray-400">
          ...
          <Link to="/signup" className="hover:underline hover:text-primary text-gray-600">
            Sign up
          </Link>
        </p>
      </div>

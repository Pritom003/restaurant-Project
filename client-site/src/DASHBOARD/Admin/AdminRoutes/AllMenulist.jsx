...
const handleUpdate = async (data) => {
  ...
  const slicedCategories = menu.slice(indexOfFirstCategory, indexOfLastCategory);
  ...
  return (
    <div className="flex flex-wrap gap-4 text-black">
                  ...
                  <div key={id} className="card p-4 border shadow-md rounded-lg">
                    ...
                    <p className="text-sm">Price: {item.price}</p>
                      ...
                      <strong>Items Included:</strong> 
                        <span >{includedItem.name} (x{includedItem.quantity}) </span>
                        </p>)) : " "}
                   
                        {item.spicyLevels.length > 0 ? item.spicyLevels.map((level, idx) => (
                          <p key={idx} className="text-sm">      
                            <strong>Spicy Levels:</strong> 
                        <span >   
                        <span key={idx}>{level.name} (+${level.price}) </span> </span>
                        </p>)) : " "}
                 

                    <p className="text-sm">
                      <strong>Varieties:</strong> 
                      {item.varieties.length > 0 ? item.varieties.map((variety, idx) => (
                        <span key={idx}>{variety.name} (+${variety.price}) </span>
                      )) : ""}
                    </p>
      ...
      <Modal className="px-5 bg-gray-300 mx-auto w-96 text-black overflow-y-scroll max-h-[80vh]" isOpen={isModalOpen} onRequestClose={closeModal} contentLabel="Update Item">
        <h2 className="text-center text-xl font-semibold mb-4">Update Item</h2>
      ...
      {editingItem.varieties.map((variety, index) => (
        <div key={index} className="flex space-x-2">
          <input
            ...
            value={variety.name}
            onChange={(e) => handleVarietyChange(index, 'name', e.target.value)} // Use handleVarietyChange here
            className="w-full p-2 border rounded bg-white"
          />
          <input
            type="number"
            placeholder="Price"
            value={variety.price}
            onChange={(e) => handleVarietyChange(index, 'price', e.target.value)} // Use handleVarietyChange here
            className="w-full p-2 border rounded bg-white"
          />
        </div>
      ))}
      <button type="button" onClick={addVariety} className="text-blue-500">Add Variety</button>
    </div>
  </div>
)}
      ...
      {editingItem.spicyLevels.map((level, index) => (
        <div key={index} className="flex space-x-2">
          <input
            ...
            value={level.name}
            onChange={(e) => handleSpicyLevelChange(index, 'name', e.target.value)} // Use handleSpicyLevelChange here
            className="w-full p-2 border rounded bg-white"
          />
          <input
            type="number"
            placeholder="Price"
            value={level.price}
            onChange={(e) => handleSpicyLevelChange(index, 'price', e.target.value)} // Use handleSpicyLevelChange here
            className="w-full p-2 border rounded bg-white"
          />
        </div>
      ))}
      <button type="button" onClick={addSpicyLevel} className="text-blue-500">Add Spicy Level</button>
    </div>
  </div>
)}
